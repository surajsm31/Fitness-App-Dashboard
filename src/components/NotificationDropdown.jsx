...
const NotificationDropdown = ({ notifications, onMarkRead, onMarkAllAsRead, onClose }) => {
    ...
    return (
        ...
        >
            ...
            <div className="overflow-y-auto h-full max-h-[calc(100vh-8rem)] xs:max-h-[calc(100vh-6rem)] sm:max-h-[28rem]">
                {notifications.length === 0 ? (
                    ...
                    <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                        {notifications.map((notification) => (
                            <li
                                key={notification.id}
                                className={`group relative p-3 xs:p-2.5 sm:p-3 lg:p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${!notification.is_read ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`}
                            >
                                <div className="flex gap-1.5 xs:gap-2 sm:gap-3 lg:gap-4">
                                    <div className="flex-shrink-0 mt-0.5">
                                        <div className="w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5 lg:w-5 lg:h-5">
                                            {getIcon(notification.type)}
                                        </div>
                                    </div>
                                    <div className="flex-1 min-w-0 space-y-0.5 xs:space-y-1">
                                        <p className={`text-xs xs:text-[10px] sm:text-xs font-medium leading-tight ${!notification.is_read ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>
                                            {notification.title}
                                        </p>
                                        <p className="text-xs xs:text-[9px] sm:text-xs text-gray-500 dark:text-gray-400 leading-snug line-clamp-1 xs:line-clamp-1 sm:line-clamp-2">
                                            {notification.message}
                                        </p>
                                        {notification.username && (
                                            <p className="text-xs xs:text-[9px] sm:text-xs text-gray-400 dark:text-gray-500 leading-tight">
                                                From: {notification.username}
                                            </p>
                                        )}
                                        <div className="flex items-center justify-between mt-1">
                                            <p className="text-[9px] xs:text-[8px] sm:text-[10px] text-gray-400 leading-tight">
                                                {notification.time}
                                            </p>
                                            {!notification.is_read && (
                                                <button
                                                    ...
                                                    className="text-xs xs:text-[10px] sm:text-xs text-primary hover:text-indigo-600 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium transition-colors"
                                                >
                                                    Mark as read
                                                </button>
                                            )}
                                    ...
                                    {!notification.is_read && (
                                        <button
                                            ...
                                            title="Mark as read"
                                        >
                                            <Check className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-3 sm:h-3" />
                                        </button>
                                    )}
                                </div>
                            </li>
