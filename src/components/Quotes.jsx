...
const Quotes = () => {
    ...
    return (
        <>
            ...
            {/* Add Quote Form */}
            {isAddingQuote && !loading && (
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Add New Quote</h3>
                        <button
                            onClick={() => {
                                setIsAddingQuote(false);
                                setNewQuote({ text: '', author: '', category: '' });
            ...
            {!loading && !error && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {currentQuotes.filter(quote => quote && quote.text).map((quote) => {
                        return (
                            <div
                                key={quote.id}
                            ...
                            >
                                ...
                                {quote.category && (
                                    <div className="flex items-center gap-1 text-xs text-primary dark:text-indigo-400">
                                        <Tag className="w-3 h-3" />
                                        <span className="truncate">{quote.category}</span>
                                    </div>
                                )}
                            </div>
                        );
